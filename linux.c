...
void tone(double frequency, double duration_ms);         // 写入音频波形
int generate(const char *ToCall, const char *FmCall, const char *Path, const char *INFO);  // 主调制生成函数

int main(int argc, char *argv[]) {

    // 检查参数
    if (argc != 5 || strcmp(argv[1], "--help") == 0) {
        printf("用法: ./aprs <'ToCall-SSID'> <'FmCall-SSID'> <'Path1-SSID、Path2-SSID'> <'INFO'>\n");
        return 1;
    }
...
// 主调制生成函数
int generate(const char *ToCallArg, const char *FmCallArg, const char *PathArg, const char *INFO) {
...
void modulate(uint8_t bit){
    uint8_t i; 
    if(bit==1){
        tone(MARK_TONE, 1000.0 / BAUD_RATE);
    }else{
        tone(SPACE_TONE, 1000.0 / BAUD_RATE);
    }
}
